...
/* ============================================ */
/* MODIFICADO: Función addNewLocation completamente reescrita */
/* ============================================ */

// Add new location - Ahora muestra opciones
function addNewLocation() {
    const modal = document.createElement('div');
    modal.innerHTML = `
        <div class="modal fade" id="newLocationTypeModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Tipo de Ubicación</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <p>Selecciona el tipo de ubicación que deseas crear:</p>
                        <div class="d-grid gap-3">
                            <button class="btn btn-outline-primary btn-lg" onclick="createStandardLocation()">
                                <i class="bi bi-list-check"></i><br>
                                <strong>Ubicación Estándar</strong><br>
                                <small>Incluye todos los productos del inventario</small>
                            </button>
                            <button class="btn btn-outline-success btn-lg" onclick="createCustomLocation()">
                                <i class="bi bi-plus-square"></i><br>
                                <strong>Ubicación Personalizada</strong><br>
                                <small>Selecciona solo los productos que necesites</small>
                            </button>
                        </div>
                    </div>
                </div>
    ...
    const modalInstance = new bootstrap.Modal(document.getElementById('newLocationTypeModal'));
    modalInstance.show();
    
    document.getElementById('newLocationTypeModal').addEventListener('hidden.bs.modal', function() {
        modal.remove();
    });
}

/* ============================================ */
/* NUEVO: Función para crear ubicación estándar */
/* ============================================ */

// Create standard location with all items
function createStandardLocation() {
    bootstrap.Modal.getInstance(document.getElementById('newLocationTypeModal')).hide();
    
    const name = prompt('Nombre de la nueva ubicación estándar:');
    if (name && !locations[name]) {
        locations[name] = {
            ...
            quantities: {},
            isCustom: false // NUEVO: Marca como ubicación estándar
        };
...
/* ============================================ */
/* NUEVO: Función para crear ubicación personalizada */
/* ============================================ */

// Create custom location with item selection
function createCustomLocation() {
    bootstrap.Modal.getInstance(document.getElementById('newLocationTypeModal')).hide();
    
    const name = prompt('Nombre de la nueva ubicación personalizada:');
    if (!name) return;
    ...
    const modal = document.createElement('div');
    modal.innerHTML = `
        <div class="modal fade" id="customLocationModal" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Seleccionar Items para "${name}"</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body" style="max-height: 60vh; overflow-y: auto;">
                        <div class="mb-3">
                            <input type="text" class="form-control" id="customLocationSearch" 
                                   placeholder="Buscar productos..." onkeyup="filterCustomLocationItems()">
                        </div>
                        <div class="mb-3">
                            <button class="btn btn-sm btn-outline-primary me-2" onclick="selectAllCustomItems()">
                                <i class="bi bi-check-all"></i> Seleccionar Todos
                            </button>
                            <button class="btn btn-sm btn-outline-secondary" onclick="deselectAllCustomItems()">
                                <i class="bi bi-x-circle"></i> Limpiar Selección
                            </button>
                            <span class="ms-3 badge bg-info" id="customSelectedCount">0 seleccionados</span>
                        </div>
                        <div id="customLocationItemsList"></div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                        <button type="button" class="btn btn-primary" onclick="saveCustomLocation('${name}')">
                            Crear Ubicación
                        </button>
                    </div>
                </div>
    ...
    const modalInstance = new bootstrap.Modal(document.getElementById('customLocationModal'));
    modalInstance.show();
    
    renderCustomLocationItems();
    
    document.getElementById('customLocationModal').addEventListener('hidden.bs.modal', function() {
        modal.remove();
        window.customLocationSelectedItems = new Set();
    });
}

/* ============================================ */
/* NUEVO: Variables y funciones para ubicaciones personalizadas */
/* ============================================ */

// Initialize custom location selected items
window.customLocationSelectedItems = new Set();

// Render items for custom location selection
function renderCustomLocationItems() {
    const container = document.getElementById('customLocationItemsList');
    container.innerHTML = '';
    ...
    inventory.forEach((item, index) => {
        ...
        grouped[item.storageLocation].push({ ...item, index });
    });
    ...
    Object.entries(grouped).forEach(([storageLocation, items]) => {
        ...
        groupDiv.setAttribute('data-family', storageLocation);
        ...
        let groupHTML = `
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <button class="btn btn-sm btn-outline-primary" onclick="toggleFamilySelection('${storageLocation}')">
                    <i class="bi bi-check-square"></i> Familia
                </button>
            </div>
        `;
        ...
        items.forEach(item => {
            groupHTML += `
                <div class="form-check mb-2 custom-location-item" data-item="${item.item.toLowerCase()}" data-family="${storageLocation}">
                    <input class="form-check-input" type="checkbox" id="item_${item.index}" 
                           onchange="toggleCustomItemSelection(${item.index})">
                    <label class="form-check-label w-100" for="item_${item.index}" style="cursor: pointer;">
...
// Toggle item selection in custom location
function toggleCustomItemSelection(itemIndex) {
    const checkbox = document.getElementById(`item_${itemIndex}`);
    
    if (checkbox.checked) {
        window.customLocationSelectedItems.add(itemIndex);
    } else {
        window.customLocationSelectedItems.delete(itemIndex);
    }
...
function toggleFamilySelection(family) {
    const familyItems = document.querySelectorAll(`[data-family="${family}"] .form-check-input`);
    const allChecked = Array.from(familyItems).every(cb => cb.checked);
    
    familyItems.forEach(checkbox => {
        const itemIndex = parseInt(checkbox.id.replace('item_', ''));
...
// Select all items in custom location
function selectAllCustomItems() {
    const allCheckboxes = document.querySelectorAll('.custom-location-item .form-check-input:not([style*="display: none"])');
    
    allCheckboxes.forEach(checkbox => {
        const itemIndex = parseInt(checkbox.id.replace('item_', ''));
        const itemDiv = checkbox.closest('.custom-location-item');
...
// Deselect all items in custom location
function deselectAllCustomItems() {
    const allCheckboxes = document.querySelectorAll('.custom-location-item .form-check-input');
    
    allCheckboxes.forEach(checkbox => {
        checkbox.checked = false;
    });
    
    window.customLocationSelectedItems.clear();
    updateCustomSelectedCount();
}

// Filter items in custom location modal
function filterCustomLocationItems() {
    const searchTerm = document.getElementById('customLocationSearch').value.toLowerCase();
    const items = document.querySelectorAll('.custom-location-item');
    
    items.forEach(item => {
        const itemName = item.getAttribute('data-item');
        const family = item.getAttribute('data-family').toLowerCase();
        
        if (itemName.includes(searchTerm) || family.includes(searchTerm)) {
    ...
    // Hide/show family headers
    const families = document.querySelectorAll('[data-family]');
    families.forEach(familyDiv => {
        if (familyDiv.classList.contains('mb-3')) {
            const visibleItems = familyDiv.querySelectorAll('.custom-location-item:not([style*="display: none"])');
            familyDiv.style.display = visibleItems.length > 0 ? 'block' : 'none';
        }
    });
}
...
function updateCustomSelectedCount() {
    const counter = document.getElementById('customSelectedCount');
    if (counter) {
        counter.textContent = `${window.customLocationSelectedItems.size} seleccionados`;
    }
}

// Save custom location
function saveCustomLocation(name) {
    if (window.customLocationSelectedItems.size === 0) {
        alert('Debes seleccionar al menos un item');
        return;
    }
    
    const selectedIndices = Array.from(window.customLocationSelectedItems);
    
    locations[name] = {
        name: name,
        locked: false,
        order: selectedIndices,
        quantities: {},
        isCustom: true // NUEVO: Marca como ubicación personalizada
    };
    ...
    bootstrap.Modal.getInstance(document.getElementById('customLocationModal')).hide();
    ...
    alert(`Ubicación personalizada "${name}" creada con ${selectedIndices.length} items`);
}

/* ============================================ */
/* FIN DE NUEVAS FUNCIONES PARA UBICACIONES PERSONALIZADAS */
/* ============================================ */

// CONTINÚA EN PARTE 2...

// CONTINUACIÓN DE script.js - PARTE 2/2
...
// Redondear números para evitar problemas de precisión
...
// Update quantity con redondeo y soporte para operaciones matemáticas
...
/* ============================================ */
/* MODIFICADO: Show location settings modal */
/* Se agrega lógica para mostrar/ocultar opciones de ubicación personalizada */
/* ============================================ */
...
function showLocationSettings(locationName) {
    ...
    // NUEVO: Mostrar/ocultar gestión de items personalizados
    const customManagement = document.getElementById('customLocationManagement');
    if (location.isCustom) {
        customManagement.style.display = 'block';
    } else {
        customManagement.style.display = 'none';
    }
...
// CONTINUACIÓN DE script.js - PARTE 3/3 FINAL

/* ============================================ */
/* NUEVO: Funciones para gestionar items en ubicaciones personalizadas */
/* ============================================ */

// Manage custom location items
function manageCustomLocationItems() {
    const location = locations[currentLocationSettings];
    
    if (!location || !location.isCustom) {
        alert('Esta función solo está disponible para ubicaciones personalizadas');
        return;
    }
    
    // Create modal for item management
    const modal = document.createElement('div');
    modal.innerHTML = `
        <div class="modal fade" id="manageCustomItemsModal" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Gestionar Items de "${currentLocationSettings}"</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body" style="max-height: 60vh; overflow-y: auto;">
                        <div class="mb-3">
                            <input type="text" class="form-control" id="manageCustomSearch" 
                                   placeholder="Buscar productos..." onkeyup="filterManageCustomItems()">
                        </div>
                        <div class="mb-3">
                            <button class="btn btn-sm btn-outline-primary me-2" onclick="selectAllManageItems()">
                                <i class="bi bi-check-all"></i> Seleccionar Todos
                            </button>
                            <button class="btn btn-sm btn-outline-secondary" onclick="deselectAllManageItems()">
                                <i class="bi bi-x-circle"></i> Limpiar Selección
                            </button>
                            <span class="ms-3 badge bg-info" id="manageSelectedCount">0 seleccionados</span>
                        </div>
                        <div id="manageCustomItemsList"></div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                        <button type="button" class="btn btn-primary" onclick="saveManageCustomLocation()">
                            Guardar Cambios
                        </button>
                    </div>
                </div>
    ...
    window.manageCustomSelectedItems = new Set(location.order);
    
    const modalInstance = new bootstrap.Modal(document.getElementById('manageCustomItemsModal'));
    modalInstance.show();
    
    renderManageCustomItems();
    
    document.getElementById('manageCustomItemsModal').addEventListener('hidden.bs.modal', function() {
        modal.remove();
        window.manageCustomSelectedItems = new Set();
    });
}

// Render items for managing custom location
function renderManageCustomItems() {
    const container = document.getElementById('manageCustomItemsList');
    container.innerHTML = '';
    ...
    Object.entries(grouped).forEach(([storageLocation, items]) => {
        ...
        let groupHTML = `
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <button class="btn btn-sm btn-outline-primary" onclick="toggleManageFamilySelection('${storageLocation}')">
                    <i class="bi bi-check-square"></i> Familia
                </button>
            </div>
        `;
        ...
        items.forEach(item => {
            const isChecked = window.manageCustomSelectedItems.has(item.index);
            groupHTML += `
                <div class="form-check mb-2 manage-custom-item" data-item="${item.item.toLowerCase()}" data-family="${storageLocation}">
                    <input class="form-check-input" type="checkbox" id="manage_item_${item.index}" 
                           ${isChecked ? 'checked' : ''}
                           onchange="toggleManageItemSelection(${item.index})">
                    <label class="form-check-label w-100" for="manage_item_${item.index}" style="cursor: pointer;">
                        <strong>${item.item}</strong>
                        <br>
                        <small class="text-muted">
                            ${item.uom ? `${item.uom}` : ''}
...
// Toggle item selection in manage modal
function toggleManageItemSelection(itemIndex) {
    const checkbox = document.getElementById(`manage_item_${itemIndex}`);
    
    if (checkbox.checked) {
        window.manageCustomSelectedItems.add(itemIndex);
    } else {
        window.manageCustomSelectedItems.delete(itemIndex);
    }
    
    updateManageSelectedCount();
}

// Toggle family selection in manage modal
function toggleManageFamilySelection(family) {
    const familyItems = document.querySelectorAll(`[data-family="${family}"] input[id^="manage_item_"]`);
    const allChecked = Array.from(familyItems).every(cb => cb.checked);
    
    familyItems.forEach(checkbox => {
        const itemIndex = parseInt(checkbox.id.replace('manage_item_', ''));
...
// Select all items in manage modal
function selectAllManageItems() {
    const allCheckboxes = document.querySelectorAll('.manage-custom-item input[type="checkbox"]');
    
    allCheckboxes.forEach(checkbox => {
        const itemIndex = parseInt(checkbox.id.replace('manage_item_', ''));
        const itemDiv = checkbox.closest('.manage-custom-item');
...
// Deselect all items in manage modal
function deselectAllManageItems() {
    const allCheckboxes = document.querySelectorAll('.manage-custom-item input[type="checkbox"]');
    ...
    window.manageCustomSelectedItems.clear();
    updateManageSelectedCount();
}

// Filter items in manage modal
function filterManageCustomItems() {
    const searchTerm = document.getElementById('manageCustomSearch').value.toLowerCase();
    const items = document.querySelectorAll('.manage-custom-item');
    ...
    families.forEach(familyDiv => {
        if (familyDiv.classList.contains('mb-3')) {
            const visibleItems = familyDiv.querySelectorAll('.manage-custom-item:not([style*="display: none"])');
            familyDiv.style.display = visibleItems.length > 0 ? 'block' : 'none';
        }
    });
}

// Update selected count in manage modal
function updateManageSelectedCount() {
    const counter = document.getElementById('manageSelectedCount');
    if (counter) {
        counter.textContent = `${window.manageCustomSelectedItems.size} seleccionados`;
    }
}

// Save managed custom location
function saveManageCustomLocation() {
    if (window.manageCustomSelectedItems.size === 0) {
        alert('Debes seleccionar al menos un item');
        return;
    }
    
    const selectedIndices = Array.from(window.manageCustomSelectedItems);
    
    // Update location order
    locations[currentLocationSettings].order = selectedIndices;
    
    // Remove quantities for items that are no longer in the location
    const newQuantities = {};
    selectedIndices.forEach(index => {
        if (locations[currentLocationSettings].quantities[index]) {
            newQuantities[index] = locations[currentLocationSettings].quantities[index];
        }
    });
    locations[currentLocationSettings].quantities = newQuantities;
    
    saveData();
    
    bootstrap.Modal.getInstance(document.getElementById('manageCustomItemsModal')).hide();
    
    if (currentLocation === currentLocationSettings) {
        showLocation(currentLocationSettings);
    }
    
    alert(`Items actualizados: ${selectedIndices.length} items en la ubicación`);
}

/* ============================================ */
/* FIN DE NUEVAS FUNCIONES PARA GESTIONAR UBICACIONES PERSONALIZADAS */
/* ============================================ */
